...
//   print(buffer);
//   // Serial.printf("Header: 0x%02x 0x%02x 0x%02x 0x%02x  |  ", shtpHeader[0], shtpHeader[1], shtpHeader[2], shtpHeader[3]);

//   uint16_t dataLength = (((uint16_t)shtpHeader[1]) << 8) | ((uint16_t)shtpHeader[0]);
//   // Serial.printf("Incoming data length: %d. Begin transmision!\n", dataLength);
//   dataLength &= ~(1 << 15); // Clear the MSbit.

//   snprintf(buffer, sizeof(buffer), "Data length after clearing the MSBIT: %d.\n", dataLength);
//   print(buffer);
//   // Serial.printf("Data length after clearing the MSBIT: %d.\n", dataLength);
//   digitalWrite(BNO08X_CS, HIGH);
